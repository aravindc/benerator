...
<#list setup.anonymizations as anon>
	...
	<generate type="${anon.varname}_map" consumer="memdb">
		<variable name="${anon.varname}" type="string" source="dom_${anon.locators[0].file}" selector="${anon.locators[0].path}" />
	...
	<iterate type="${locator.entity}" source="dom_${locator.file}" selector="${locator.entityPath}" consumer="dom_${locator.file}.updater()">
		<variable name="${anon.varname}_map" source="memdb" type="${anon.varname}_map" selector="_candidate.source==this.${locator.attribute}"/>
